...
Este projeto foi desenvolvido como um desafio prático de consolidação de conceitos durante o curso **DEVStart - HTML, CSS e JavaScript**, com o foco em aplicar na prática os fundamentos do desenvolvimento front-end web nativo e design de interface (UI).
...
- **Seções de Conteúdo Estruturadas:**
  - **Planejamento de Jornada:** Detalhes cruciais sobre o roteiro de 10 dias passando por Cracóvia (Polônia), Budapeste (Hungria) e Praga (República Tcheca).
  - **Conecte-se Conosco:** Central de atendimento exibindo canais de e-mail, telefone e suporte personalizado 24/7 via WhatsApp.
...
- **HTML5:** Estruturação semântica através do uso correto de tags como `<nav>`, `<main>`, `<section>`, `<article>` e `<footer>`.
- **CSS3:** Estilização customizada avançada incluindo:
  - Reset global de margens e aplicação do modelo de caixa inteligente (`box-sizing: border-box`).
  - Posicionamento moderno (`position: sticky` e `display: flex` para centralização do banner).
